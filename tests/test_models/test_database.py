...
def test_update_envelope(db):
    ...
    env_id = db.insert_envelope("Shopping", 300.00, 50.00, "Clothing and other shopping")
    updated = db.update_envelope(env_id, category="Online Shopping", budgeted_amount=350.00, description="Online purchases")
...
def test_delete_envelope(db):
    ...
    env_id = db.insert_envelope("Entertainment", 100.00, 10.00, "Movies, concerts, etc.")
...
def test_insert_and_get_transaction(db):
    ...
    trans_id = db.insert_transaction(env_id, 25.50, "Lunch", date(2023, 1, 15), "expense")
...
def test_get_transactions_for_envelope(db):
    ...
    db.insert_transaction(env_id1, 50.00, "Friend's Birthday", date(2023, 2, 10), "expense")
    db.insert_transaction(env_id2, 200.00, "Flight tickets", date(2023, 2, 15), "expense")
    db.insert_transaction(env_id1, 30.00, "Office gift exchange", date(2023, 2, 20), "expense")
...
def test_get_all_transactions(db):
    ...
    db.insert_transaction(env_id1, 45.00, "Weekly groceries", date(2023, 3, 1), "expense")
...
def test_update_transaction(db):
    ...
    trans_id = db.insert_transaction(env_id, 75.00, "Electricity Bill", date(2023, 4, 5), "expense")

    env_id_new = db.insert_envelope("Utilities", 200.00, 0.00, "New Utilities")
    updated = db.update_transaction(trans_id, envelope_id=env_id_new, amount=80.00, description="Updated Electricity Bill", date=date(2023, 4, 6), type="expense")
    assert updated is True
    transaction = db.get_transaction_by_id(trans_id)
    ...
    assert transaction["date"] == "2023-04-06" # Dates are stored and retrieved as ISO format strings

def test_delete_transaction(db):
    # Test deleting a transaction
    env_id = db.insert_envelope("Subscriptions", 50.00, 5.00, "Streaming services")
    trans_id = db.insert_transaction(env_id, 10.00, "Music Subscription", date(2023, 5, 1), "expense")
...
def test_insert_transaction_invalid_envelope_id_raises_value_error(db):
    ...
    with pytest.raises(ValueError) as excinfo:
        db.insert_transaction(non_existent_env_id, 50.00, "Test Transaction", date(2023, 1, 1), "expense")
    assert f"Envelope with ID {non_existent_env_id} does not exist" in str(excinfo.value)
...
def test_get_envelope_current_balance_with_expenses(db):
    ...
    db.insert_transaction(env_id, 30.00, "Dinner with friends", date(2023, 6, 5), "expense")
    balance = db.get_envelope_current_balance(env_id)
...
def test_get_envelope_current_balance_with_income(db):
    ...
    db.insert_transaction(env_id, 150.00, "Web design project", date(2023, 6, 10), "income")
    db.insert_transaction(env_id, 200.00, "Tutoring session", date(2023, 6, 12), "income")
    balance = db.get_envelope_current_balance(env_id)
...
def test_get_envelope_current_balance_mixed_transactions(db):
    ...
    db.insert_transaction(env_id, 500.00, "Salary deposit", date(2023, 7, 1), "income")    # Balance = 1500
    db.insert_transaction(env_id, 100.00, "Groceries", date(2023, 7, 2), "expense")      # Balance = 1400
    db.insert_transaction(env_id, 50.00, "Refund for returned item", date(2023, 7, 3), "income") # Balance = 1450
    db.insert_transaction(env_id, 200.00, "Rent payment", date(2023, 7, 5), "expense")    # Balance = 1250
    balance = db.get_envelope_current_balance(env_id)
