...
class TestMotherDuckCloudMode:
    ...
    @patch('app.models.database.duckdb.connect')
    def test_cloud_mode_db_creation_failure_main_connection_fallback(self, mock_connect):
        ...
        mock_connect.side_effect = [
            duckdb.Error("MotherDuck DB creation failed"),  # Creation fails (logged only)
            duckdb.Error("MotherDuck main connection failed"),  # Main connection fails (triggers fallback)
    ...
    def test_cloud_mode_connection_status_fallback_warning(self):
        ...
        with patch('app.models.database.duckdb.connect') as mock_connect:
            ...
            mock_connect.side_effect = [
                ...
                duckdb.Error("Main connection failed"),  # Main connection fails (triggers fallback)
            ...
            assert status['warning'] == "Requested cloud mode but fell back to local-only connection"
    ...
    def test_cloud_mode_no_token_config_validation(self, mock_connect):
        ...
        with pytest.raises(ValueError, match="MotherDuck token is required for 'cloud' mode"):
            Database(
                db_path='test.db', 
                mode='cloud', 
                motherduck_config={}  # No token
            )
