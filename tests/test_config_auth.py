...
class TestStartupValidation:
    ...
    @patch('builtins.print')
    @patch('sys.exit')
    def test_run_main_continues_with_valid_bearer_token(self, mock_exit, mock_print, mock_create_server):
        ...
        with patch.dict(os.environ, {'BEARER_TOKEN': 'valid-token-123'}):
            ...
            with patch('app.config.Config.ensure_data_directory'):
                ...
                if mock_exit.called:
                    ...
                    assert args[0] != 1 or not any("BEARER_TOKEN" in str(call) for call in mock_print.call_args_list)
