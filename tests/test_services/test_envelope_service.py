...
from app.models.database import Database # Assuming Database is the class used by the service
...
def mock_db():
    ...
    db.get_envelope_by_id.return_value = {
        'id': 1, 'category': 'Test', 'budgeted_amount': 100,
        'starting_balance': 50, 'description': 'Test desc', 'current_balance': 50
...
def test_create_envelope_success(envelope_service, mock_db):
    ...
    expected_envelope = {
        "id": 123, "category": category, "budgeted_amount": budgeted_amount,
        "starting_balance": starting_balance, "description": description, "current_balance": 50.0
    }
    mock_db.get_envelope_by_id.return_value = expected_envelope
    mock_db.get_envelope_current_balance.return_value = 50.0 # Assuming this is called by get_envelope

    created_envelope = envelope_service.create_envelope(category, budgeted_amount, starting_balance, description)

    mock_db.get_envelope_by_category.assert_called_once_with(category)
    mock_db.insert_envelope.assert_called_once_with(category, budgeted_amount, starting_balance, description)
    mock_db.get_envelope_by_id.assert_called_once_with(123) # Ensure get_envelope was called with the new ID
...
def test_create_envelope_category_exists(envelope_service, mock_db):
    ...
    with pytest.raises(ValueError) as excinfo:
        envelope_service.create_envelope(category, budgeted_amount, starting_balance, description)

    assert f"Envelope with category '{category}' already exists." in str(excinfo.value)
    mock_db.insert_envelope.assert_not_called()

@pytest.mark.parametrize("category, budgeted_amount, starting_balance, expected_message", [
...
def test_create_envelope_invalid_input(envelope_service, mock_db, category, budgeted_amount, starting_balance, expected_message):
    with pytest.raises(ValueError) as excinfo:
        envelope_service.create_envelope(category, budgeted_amount, starting_balance, "Test description")
...
def test_get_envelope_success(envelope_service, mock_db):
    envelope_id = 1
    expected_db_envelope = {"id": envelope_id, "category": "Food", "budgeted_amount": 150, "starting_balance": 20, "description": "Food stuff"}
...
def test_get_all_envelopes_success(envelope_service, mock_db):
    db_envelopes = [
        {"id": 1, "category": "Rent", "budgeted_amount": 1000, "starting_balance": 1000, "description": ""},
        {"id": 2, "category": "Gas", "budgeted_amount": 100, "starting_balance": 50, "description": ""}
    ]
    ...
    mock_db.get_envelope_current_balance.side_effect = lambda id_val: balances.get(id_val, 0)
...
def test_update_envelope_success_all_fields(envelope_service, mock_db):
    ...
    mock_db.get_envelope_current_balance.return_value = updated_envelope_data["current_balance"]
...
def test_update_envelope_success_partial_fields(envelope_service, mock_db):
    ...
    final_envelope_state = {
        "id": envelope_id, "category": "Groceries", "budgeted_amount": 300.0,
        "starting_balance": 50.0, "description": "Test desc", "current_balance": 50.0
    }
    mock_db.get_envelope_by_id.return_value = final_envelope_state
    mock_db.get_envelope_current_balance.return_value = final_envelope_state["current_balance"]

    updated_envelope = envelope_service.update_envelope(envelope_id, **update_data)

    mock_db.get_envelope_by_category.assert_not_called() # Category not in update_data
    mock_db.update_envelope.assert_called_once_with(envelope_id, category=None, budgeted_amount=300.0, starting_balance=None, description=None)
    assert updated_envelope["budgeted_amount"] == 300.0

def test_update_envelope_category_already_exists_for_another_envelope(envelope_service, mock_db):
    envelope_id = 1
    ...
    assert f"Envelope with category '{new_category}' already exists." in str(excinfo.value)
    mock_db.update_envelope.assert_not_called()

def test_update_envelope_category_already_exists_for_same_envelope_no_change(envelope_service, mock_db):
    envelope_id = 1
    ...
    mock_db.get_envelope_by_category.return_value = {"id": envelope_id, "category": current_category}
    ...
    expected_envelope_after_update = {
        "id": envelope_id, "category": current_category, "budgeted_amount": 100,
        "starting_balance": 50, "description": "Test desc", "current_balance": 50
    }
    ...
    updated_envelope = envelope_service.update_envelope(envelope_id, category=current_category, budgeted_amount=100)

    mock_db.get_envelope_by_category.assert_called_once_with(current_category)
    mock_db.update_envelope.assert_called_once_with(envelope_id, category=current_category, budgeted_amount=100, starting_balance=None, description=None)
    assert updated_envelope["category"] == current_category


@pytest.mark.parametrize("field, value, expected_message_part", [
    ("category", "", "Category must be a non-empty string."),
    ("category", "  ", "Category must be a non-empty string."),
    ("budgeted_amount", -100, "Budgeted amount must be a non-negative number."),
    ("budgeted_amount", "invalid", "Budgeted amount must be a non-negative number."),
    ("starting_balance", "invalid", "Starting balance must be a number."),
])
def test_update_envelope_invalid_input_types(envelope_service, mock_db, field, value, expected_message_part):
    envelope_id = 1
...
def test_update_envelope_db_update_fails_returns_false(envelope_service, mock_db):
    ...
    mock_db.update_envelope.return_value = False # Simulate DB update failure (e.g. row not found)
...
def test_delete_envelope_success(envelope_service, mock_db):
    ...
    mock_db.get_envelope_by_id.assert_called_once_with(envelope_id) # Service checks existence first
    mock_db.delete_envelope.assert_called_once_with(envelope_id)
    assert result == {"message": f"Envelope with ID {envelope_id} deleted successfully."}
