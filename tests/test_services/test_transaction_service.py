...
def mock_db():
    ...
    db.get_envelope_by_id.return_value = {"id": 1, "name": "Test Envelope"} # Default: envelope exists
    db.insert_transaction.return_value = 101 # Default: successful insertion returns ID 101
    # Default: mock fetched transaction
    db.get_transaction_by_id.return_value = {
        "id": 101, "envelope_id": 1, "amount": 50,
        "description": "Test transaction", "date": "2024-01-01", "type": "expense"
    }
    return db
...
def test_create_transaction_success(transaction_service, mock_db):
    ...
    mock_db.get_envelope_by_id.return_value = {"id": envelope_id, "name": "Food Envelope"}
    ...
    expected_transaction = {
        "id": 201, "envelope_id": envelope_id, "amount": amount,
        "description": description, "date": date, "type": type
    }
    mock_db.get_transaction_by_id.return_value = expected_transaction

    created_transaction = transaction_service.create_transaction(envelope_id, amount, description, date, type)

    mock_db.get_envelope_by_id.assert_called_once_with(envelope_id)
    mock_db.insert_transaction.assert_called_once_with(envelope_id, amount, description, date, type)
    mock_db.get_transaction_by_id.assert_called_once_with(201) # Ensure get_transaction was called
...
def test_create_transaction_envelope_not_found(transaction_service, mock_db):
    ...
    with pytest.raises(ValueError) as excinfo:
        transaction_service.create_transaction(envelope_id, 50, "Test", "2024-01-01", "expense")

    assert f"Envelope with ID {envelope_id} does not exist." in str(excinfo.value)
    mock_db.insert_transaction.assert_not_called()

@pytest.mark.parametrize("amount, date, type, expected_message", [
    (0, "2024-01-01", "expense", "Amount is required and must be a positive number."),
    (-10, "2024-01-01", "expense", "Amount is required and must be a positive number."),
    ("abc", "2024-01-01", "expense", "Amount is required and must be a positive number."),
...
def test_create_transaction_invalid_input(transaction_service, mock_db, amount, date, type, expected_message):
    envelope_id = 1
    # Ensure envelope exists for these tests, so other validations are triggered
    mock_db.get_envelope_by_id.return_value = {"id": envelope_id, "name": "Test Envelope"}

    with pytest.raises(ValueError) as excinfo:
        transaction_service.create_transaction(envelope_id, amount, "Test desc", date, type)
...
def test_get_transaction_success(transaction_service, mock_db):
    ...
    expected_db_transaction = {
        "id": transaction_id, "envelope_id": 1, "amount": 75,
        "description": "Dinner", "date": "2024-07-26", "type": "expense"
    }
...
def test_get_transactions_by_envelope_success(transaction_service, mock_db):
    ...
    expected_db_transactions = [
        {"id": 101, "envelope_id": envelope_id, "amount": 50, "description": "Gas", "date": "2024-01-01", "type": "expense"},
        {"id": 102, "envelope_id": envelope_id, "amount": 120, "description": "Salary", "date": "2024-01-02", "type": "income"}
    ]
    ...
    mock_db.get_envelope_by_id.assert_called_once_with(envelope_id) # Service checks if envelope exists
...
def test_get_transactions_by_envelope_empty(transaction_service, mock_db):
    envelope_id = 2
    mock_db.get_envelope_by_id.return_value = {"id": envelope_id, "name": "Empty Envelope"}
...
def test_get_all_transactions_success(transaction_service, mock_db):
    all_db_transactions = [
        {"id": 101, "envelope_id": 1, "amount": 50, "description": "Gas", "date": "2024-01-01", "type": "expense"},
        {"id": 201, "envelope_id": 2, "amount": 1200, "description": "Salary", "date": "2024-01-02", "type": "income"}
    ]
...
def test_update_transaction_success_all_fields(transaction_service, mock_db):
    ...
    mock_db.get_envelope_by_id.return_value = {"id": update_data["envelope_id"], "name": "New Envelope"}
    ...
    mock_db.get_transaction_by_id.assert_called_once_with(transaction_id) # From service's get_transaction call
...
def test_update_transaction_success_partial_fields(transaction_service, mock_db):
    ...
    final_transaction_state = {
        "id": transaction_id, "envelope_id": 1, "amount": 50,
        "description": "Just a new description", "date": "2024-01-01", "type": "expense"
    }
    ...
    expected_call_args = {
        "envelope_id": None, "amount": None, "description": "Just a new description",
        "date": None, "type": None
    }
...
def test_update_transaction_target_envelope_not_found(transaction_service, mock_db):
    ...
    assert f"Envelope with ID {invalid_envelope_id} does not exist." in str(excinfo.value)
...
def test_update_transaction_invalid_input_types(transaction_service, mock_db, field, value, expected_message_part):
    transaction_id = 104
...
def test_delete_transaction_success(transaction_service, mock_db):
    ...
    assert result == {"message": f"Transaction with ID {transaction_id} deleted successfully."}
