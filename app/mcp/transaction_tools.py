...
class TransactionTools:
    ...
    async def create_transaction(
        ...
        try:
            ...
            return [TextContent(type="text", text=json.dumps(transaction, indent=2))]
            ...
            logger.exception("An internal error occurred in create_transaction")
            return [
                TextContent(
                    type="text",
    ...
    async def list_transactions(
        ...
        envelope_id: Annotated[
            Optional[int], "Optional envelope ID to filter transactions"
        ] = None,
    ) -> list[TextContent]:
        """Get all transactions, optionally filtered by envelope."""
        try:
            if envelope_id:
                transactions = self.transaction_service.get_transactions_by_envelope(
                    envelope_id
                )
            else:
    ...
    async def get_transaction(
        self, transaction_id: Annotated[int, "ID of the transaction to retrieve"]
    ) -> list[TextContent]:
        """Get specific transaction details by ID."""
        try:
            transaction = self.transaction_service.get_transaction(transaction_id)
            if not transaction:
                return [TextContent(type="text", text="Error: Transaction not found")]
            return [TextContent(type="text", text=json.dumps(transaction, indent=2))]
        except (TypeError, KeyError, AttributeError) as e:
            return [
                TextContent(
    ...
    async def delete_transaction(
        self, transaction_id: Annotated[int, "ID of the transaction to delete"]
    ) -> list[TextContent]:
